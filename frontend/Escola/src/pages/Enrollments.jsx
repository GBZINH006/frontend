...
export default function Enrollments() {
  ...
  const save = async () => {
    try {
      ...
      show({ severity: "success", summary: "Matrícula realizada", detail: "Aluno matriculado com sucesso.", life: 3000 });
      setDialog(false);
      setForm(empty);
      load();
    } catch (e) {
      show({ severity: "error", summary: "Erro", detail: e.response?.data?.message || "Erro ao matricular.", life: 4000 });
    }
  };
  ...
  const remove = (row) => {
    confirmDialog({
      ...
      accept: async () => {
        ...
        show({ severity: "warn", summary: "Matrícula cancelada", detail: "Matrícula removida.", life: 3000 });
        load();
      },
    });
  };
  ...
  return (
    <div className="animate-in">
      ...
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => { setForm(empty); setDialog(true); }}>
      ...
      <div className="card">
        ...
        <DataTable value={filtered} loading={loading} paginator rows={10} emptyMessage="Nenhuma matrícula encontrada." rowHover>
      ...
      <Dialog header="Nova Matrícula" visible={dialog} onHide={() => setDialog(false)} style={{ width: 460 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingTop: 8 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <button className="btn btn-primary" onClick={save} disabled={!form.student_id || !form.class_id}>
              Matricular
            </button>
          </div>
        </div>
