...
export default function Students() {
  ...
  const openEdit = (row) => { setForm({ name: row.name, email: row.email }); setEditing(row.id); setDialog(true); };

  const save = async () => {
    try {
      if (editing) {
        await api.put(`/students/${editing}`, form);
        show({ severity: "success", summary: "Atualizado", detail: "Aluno atualizado.", life: 3000 });
      }
      setDialog(false);
      load();
    } catch (e) {
  ...
  const remove = (row) => {
    confirmDialog({
      ...
      accept: async () => {
        ...
        show({ severity: "warn", summary: "Excluído", detail: "Aluno removido.", life: 3000 });
        load();
      },
    });
  };
